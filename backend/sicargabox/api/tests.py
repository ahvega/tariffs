...
from MiCasillero.models import PartidaArancelaria, Cliente, Cotizacion, Articulo, ParametroSistema
...
class PartidaArancelariaViewSetTestCase(TestCase):
    ...
    def test_filter_by_courier_category(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get('/api/partidas-arancelarias/?courier_category=RESTRICTED')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_products_endpoint_public(self):
        ...
        response = self.client.get('/api/partidas-arancelarias/search_products/?query=Electronics')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
