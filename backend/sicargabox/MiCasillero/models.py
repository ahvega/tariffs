        ...
        try:
            ...
            if parametro_obj.tipo_dato == "STRING":
                ...
                raise ValidationError(
                    f"Tipo de dato no soportado: {parametro_obj.tipo_dato}"
                )
...
class PartidaArancelaria(models.Model):
    ...
    impuesto_dai = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0,
    ...
    impuesto_isc = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0,
    ...
    impuesto_ispc = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0,
    ...
    impuesto_isv = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0,
    ...
    restrictions = models.JSONField(
        default=list, blank=True, verbose_name="Restricciones"
    )  # Lista de restricciones específicas
    package_type = models.CharField(
        max_length=100, blank=True, verbose_name="Tipo de Paquete"
    )
    max_weight_allowed = models.DecimalField(
        max_digits=8,
        decimal_places=2,
        null=True,
        blank=True,
    ...
    hierarchy_level = models.IntegerField(
        default=4,
        help_text=(
            "Nivel de jerarquía (1=Capítulo, 2=Encabezado, "
            "3=Subencabezado, 4=Partida detallada)"
        ),
    ...
    def save(self, *args, **kwargs):
        ...
        if self.search_keywords is None:
            ...
            try:
                ...
                self.search_keywords = (
                    [self.search_keywords] if self.search_keywords.strip() else []
                )
        ...
        if not kwargs.get("update_fields"):
            PartidaArancelaria.objects.filter(pk=self.pk).update(
                search_vector=SearchVector("descripcion", weight="A")
    ...
    def generate_search_keywords(self):
        ...
        keywords = [self.descripcion, self.item_no, self.partida_arancelaria]
...
class Cliente(models.Model):
    ...
    nombre_corto = models.CharField(
        max_length=50, blank=True, verbose_name="Nombre Corto"
    )  # Este campo se generará programáticamente
    direccion = models.CharField(max_length=255, verbose_name="Dirección")
    ciudad = models.CharField(
        max_length=50, default="San Pedro Sula", verbose_name="Ciudad"
    )
    departamento = models.CharField(
        max_length=50, default="Cortés", verbose_name="Departamento"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.nombre_corto:
            if self.nombres and self.apellidos:
                nombres_iniciales = "".join(
                    [n[0].upper() + "." for n in self.nombres.split() if n]
                )
                apellidos = self.apellidos.split()
                if apellidos:
                    apellidos_iniciales = "".join(
                        [a[0].upper() for a in apellidos[1:] if a]
                    )
                    self.nombre_corto = (
                        f"{nombres_iniciales}{apellidos[0]} {apellidos_iniciales}"
                    )
                else:
        ...
        if not self.codigo_cliente:
            # Remove force_insert for the first save since we'll need to
            # update the codigo_cliente
            kwargs_first_save = kwargs.copy()
            kwargs_first_save.pop("force_insert", None)
            super().save(
                *args, **kwargs_first_save
            )  # Guardar primero para obtener el ID
            prefijo_any = ParametroSistema.objects.get_valor(
                "Prefijo del Código de Cliente"
            )
            if prefijo_any is None:
                raise ValidationError(
                    "El parámetro 'Prefijo del Código de Cliente' no está definido."
                )
...
User.add_to_class(
    "cliente", property(lambda u: Cliente.objects.get_or_create(user=u)[0])
)
...
class Cotizacion(models.Model):
    ...
    )  # Auto-calculate (fecha_creacion + días de validez from ParametroSistema)
    ...
    subtotal_articulos = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Subtotal Artículos"
    )
    total_flete = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Total Flete"
    )
    total_impuestos = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Total Impuestos"
    )
    total_estimado = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Total Estimado"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if dias_validez is None:
            raise ValidationError(
                "El parámetro 'Días Validez Cotización' no está definido."
            )
        try:
            dias_validez = int(dias_validez)
        except (TypeError, ValueError):
            raise ValidationError(
                "El parámetro 'Días Validez Cotización' debe ser un número entero."
            )
        ...
        # Asignar permisos de objeto a los operadores y administradores
        permissions = ["view", "change"]
        if "Administradores" in [
            group.name for group in self.cliente.user.groups.all()
        ]:
            permissions.append("delete")
...
class Envio(models.Model):
    ...
    cotizacion = models.ForeignKey(
        Cotizacion, on_delete=models.CASCADE, verbose_name="Cotización"
    )
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, verbose_name="Cliente"
    )
    tracking_number_original = models.CharField(
        max_length=50, verbose_name="Tracking No. Original"
    )  # From retailer
    tracking_number_sicarga = models.CharField(
    ...
    peso_estimado = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name="Peso Estimado"
    )
    peso_real = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, verbose_name="Peso Real"
    )
    largo_real = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Largo Real"
    )
    ancho_real = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Ancho Real"
    )
    alto_real = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Alto Real"
    )
    factura_compra = models.FileField(
        upload_to="invoices/", null=True, blank=True, verbose_name="Factura de Compra"
    )
    foto_paquete = models.ImageField(
        upload_to="packages/", null=True, blank=True, verbose_name="Foto del Paquete"
    )
    ...
    fecha_recibido_miami = models.DateTimeField(
        null=True, blank=True, verbose_name="Fecha de Recepción en Miami"
    )
    fecha_salida_miami = models.DateTimeField(
        null=True, blank=True, verbose_name="Fecha de Salida de Miami"
    )
    fecha_llegada_honduras = models.DateTimeField(
        null=True, blank=True, verbose_name="Fecha de Llegada a Honduras"
    )
    fecha_liberacion_aduana = models.DateTimeField(
        null=True, blank=True, verbose_name="Fecha de Liberación de Aduana"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        costo_por_libra = ParametroSistema.objects.get_valor(
            "Costo Flete por Libra en USD$"
        )

        if costo_por_libra is None:
            raise ValidationError(
                "El parámetro 'Costo Flete por Libra en USD$' no está definido."
            )
        try:
            costo_decimal = Decimal(str(costo_por_libra))
        except Exception:
            raise ValidationError(
                "El parámetro 'Costo Flete por Libra en USD$' no es "
                "convertible a Decimal."
            )
        ...
        permissions = ["view", "change"]
        if "Administradores" in [
            group.name for group in self.cotizacion.cliente.user.groups.all()
        ]:
            permissions.append("delete")
...
class StatusUpdate(models.Model):
    ...
    estado_anterior = models.CharField(
        max_length=50, blank=True, verbose_name="Estado Anterior"
    )
    estado_nuevo = models.CharField(max_length=50, verbose_name="Estado Nuevo")
    actualizado_por = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, verbose_name="Actualizado Por"
    )
...
class Factura(models.Model):
    ...
    flete = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True, verbose_name="Flete"
    )
    total_impuesto_dai = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    total_impuesto_isc = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    total_impuesto_ispc = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    total_impuesto_isv = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    estado_pago = models.CharField(
        max_length=20, choices=ESTADO_PAGO_CHOICES, verbose_name="Estado de Pago"
    )
...
class Alerta(models.Model):
    ...
    tipo_alerta = models.CharField(
        max_length=50, choices=TIPO_ALERTA_CHOICES, verbose_name="Tipo de Alerta"
    )
    mensaje = models.TextField(verbose_name="Mensaje")
    fecha_envio = models.DateTimeField(auto_now_add=True, verbose_name="Fecha de Envío")
    estado = models.CharField(
        max_length=20, choices=ESTADO_ALERTA_CHOICES, verbose_name="Estado"
    )
...
class Articulo(models.Model):
    ...
    descripcion_original = models.CharField(
        max_length=255,
        help_text="Descripción del producto según la factura de compra",
        verbose_name="Descripción Original",
    )
    valor_articulo = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Valor Artículo"
    )
    ...
    partida_arancelaria = models.ForeignKey(
        PartidaArancelaria, on_delete=models.CASCADE, verbose_name="Partida Arancelaria"
    )
    impuesto_dai = models.DecimalField(
        max_digits=10,
        decimal_places=2,
    ...
    impuesto_total = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    ai_suggested_partidas = models.JSONField(
        ...
        blank=True,
        help_text=(
            "Top 5 sugerencias del sistema AI: " "[{partida_id, score, reason}, ...]"
        ),
    ...
    def calcular_impuestos(self):
        ...
        # Cálculo incremental de impuestos (cada impuesto se calcula
        # sobre el total acumulado anterior)
        ...
        impuesto_total = impuesto_dai + impuesto_isc + impuesto_ispc + impuesto_isv
...
class ItemPartidaMapping(models.Model):
    ...
    def __str__(self):
        desc = self.item_description_original[:50]
        return f"{desc} → {self.partida_arancelaria.item_no}"
