...
def create_user_groups(apps, schema_editor):
    ...
    if created:
        ...
        content_type_cliente = ContentType.objects.get_for_model(apps.get_model('MiCasillero', 'Cliente'))
        permissions_cliente = Permission.objects.filter(content_type=content_type_cliente)
    ...
    if created:
        ...
        content_types_operador = [
            ...
            for model in ['Cliente', 'Cotizacion', 'Articulo', 'Envio', 'Factura', 'Alerta']
        ]
        permisos_operador = Permission.objects.filter(content_type__in=content_types_operador).exclude(
            codename__in=['delete_cliente'])
    ...
    if created:
        ...
        permisos_vendedor = Permission.objects.filter(content_type__in=content_types_vendedor).exclude(
            codename__in=['delete_cliente', 'add_cliente'])
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        ('auth', '0012_alter_user_first_name_max_length'),  # Ajustado según la última migración disponible
    ]
