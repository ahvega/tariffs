...
class Command(BaseCommand):
    help = (
        "Actualiza las descripciones de las partidas arancelarias desde el archivo CSV"
    )
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.ERROR(f"No se encontró el archivo CSV en: {csv_path}")
            )
            self.stdout.write(
                self.style.ERROR(
                    "Asegúrate de ejecutar el comando desde el directorio backend/sicargabox"
                )
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                f"Archivo CSV leído correctamente. Total de registros: {len(df)}"
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Error al leer el archivo CSV: {str(e)}")
            )
            return
                ...
                if partida.item_no in descriptions_dict:
                    ...
                    if partida.descripcion != new_description:
                        if dry_run:
                            ...
                            self.stdout.write(
                                f"Descripción actual: {partida.descripcion}"
                            )
