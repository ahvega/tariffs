...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--start-index', type=int, default=0, help='Start from specific index')
        parser.add_argument('--dry-run', action='store_true', help='Run without saving to database')
        parser.add_argument('--api-provider', type=str, choices=['openai', 'deepseek'], default='deepseek',
    ...
    def classify_description(self, client, descripcion, codigo, model_name):
        ...
        try:
            response = client.chat.completions.create(
                model=model_name,
                messages=[
                    {"role": "system", "content": "Eres un experto en aduanas y logística especializado en envíos por courier."},
    ...
    def _validate_classification_result(self, result):
        ...
        # Clean string fields
        string_fields = ['category', 'package_type', 'special_instructions', 'reasoning']
        ...
        result['requires_special_handling'] = bool(result.get('requires_special_handling', True))
        ...
        result['restrictions'] = [
            str(r).replace('\n', ' ').replace('\r', '')
                ...
                .replace('\\', '\\\\')
                .strip()
            for r in result['restrictions'] if r
        ]
    ...
    def process_row(self, row, client, model_name):
        try:
            ...
            descripcion = str(row['partida']).strip()  # Using the extended description from 'partida'
            ...
            level = str(row['nivel']).strip() if 'nivel' in row else '4'  # Default to level 4 if not specified

            # Get classification from API
            classification_result = self.classify_description(client, descripcion, codigo, model_name)
            ...
            courier_category = category_mapping.get(classification_result.get('category', 'PROHIBIDO'), 'PROHIBITED')
            ...
            return {
                ...
                'restrictions': classification_result.get('restrictions', ['Sin clasificación']),
                'package_type': classification_result.get('package_type', 'NO_APLICA'),
                'requires_special_handling': bool(classification_result.get('requires_special_handling', True)),
                'special_instructions': classification_result.get('special_instructions', 'Requiere revisión manual'),
                'max_weight_allowed': classification_result.get('max_weight_allowed'),
                'search_keywords': f"{descripcion} {codigo}".strip() or 'Sin palabras clave'  # Ensure non-null
            }
        except Exception as e:
    ...
    def handle(self, *args, **options):
        ...
        if options['api_provider'] == 'deepseek':
            ...
            if not api_key:
                raise CommandError('DEEPSEEK_API_KEY not found in environment variables. Please add it to your .env file.')
            ...
            if not api_key:
                raise CommandError('OPENAI_API_KEY not found in environment variables. Please add it to your .env file.')
        ...
        self.stdout.write(f'Using {options["api_provider"]} API with model {model_name}')
        if options['api_provider'] == 'deepseek':
        ...
        existing_records = set(PartidaArancelaria.objects.values_list('item_no', flat=True))
        ...
        if remaining_rows == 0:
            self.stdout.write(self.style.SUCCESS('All records have already been processed. Nothing new to import.'))
            return
            
        self.stdout.write(f'Processing {remaining_rows} new items starting from index {options["start_index"]}')
                ...
                if not options['dry_run']:
                    try:
                        ...
                        if len(batch) >= options['batch_size']:
                            ...
                            self.stdout.write(f'Saved batch of {len(batch)} items. Progress: {saved}/{processed} processed')
                            batch = []
                            
                    except Exception as e:
                        error_msg = f"Error saving item {item_data['item_no']}: {str(e)}"
                        logger.error(error_msg)
                        self.stdout.write(self.style.ERROR(error_msg))
                        self.stdout.write(self.style.ERROR("Aborting import due to validation error. Please fix the issues and try again."))
                        raise CommandError(error_msg)
                else:
                    self.stdout.write(f"Would save item: {item_data['item_no']} - {item_data.get('courier_category', 'UNKNOWN')}")
                    saved += 1
