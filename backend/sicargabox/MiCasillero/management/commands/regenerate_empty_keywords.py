...
class Command(BaseCommand):
    ...
    def generate_keywords_with_ai(self, context, api_provider):
        ...
        try:
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "Eres un experto en clasificación arancelaria y comercio internacional. Responde solo con arrays JSON puros, sin formato markdown."},
    ...
    def handle(self, *args, **options):
        ...
        if total_partidas == 0:
            self.stdout.write(self.style.SUCCESS('No hay partidas con keywords vacíos para procesar.'))
            return
