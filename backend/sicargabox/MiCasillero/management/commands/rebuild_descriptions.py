...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--output-file', type=str, default='aranceles_jerarquia.csv', help='Output CSV file path')
    ...
    def build_hierarchical_description(self, row, df):
        ...
        if not parent_mask.any():
            self.stdout.write(f"Warning: Parent item not found for code {parent_code} (child: {row['código']})")
            return row['descripción']
        parent_item = df[parent_mask].iloc[0]
        self.stdout.write(f"Found parent item: {parent_item['código']} - {parent_item['descripción'][:50]}...")
        ...
        if not grandparent_mask.any():
            self.stdout.write(f"Warning: Grandparent item not found for code {grandparent_code} (parent: {parent_code})")
            return f"{parent_item['descripción']} - {row['descripción']}"
        grandparent_item = df[grandparent_mask].iloc[0]
        self.stdout.write(f"Found grandparent item: {grandparent_item['código']} - {grandparent_item['descripción'][:50]}...")
        ...
        self.stdout.write(f"Successfully built description for {row['código']}: {complete_desc[:100]}...")
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            df['descripción_completa'] = df.apply(lambda row: self.build_hierarchical_description(row, df), axis=1)
            ...
            if level4_count > 0:
                df.loc[level4_mask, 'descripción'] = df.loc[level4_mask, 'descripción_completa']
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully processed {len(df)} items'))
