    ...
    def classify_description(self, client, descripcion):
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a customs and logistics expert."},
    ...
    def handle(self, *args, **options):
        ...
        for i in range(0, len(df), batch_size):
            ...
            for _, row in batch.iterrows():
                try:
                    classification = self.classify_description(client, row['descripcion'])
                    ...
                    if classification['suitable_for_courier']:
                        partida = PartidaArancelaria(
                            ...
                            impuesto_ispc=float(row['ispc']) if row['ispc'] != '-' else 0,
